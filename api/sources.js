  ...
  try {
    ...
    (function walk(n) {
      ...
      if (title && url)
        out.push({
          title,
          link: absUrl(url, BASE_RC),
          image: absUrl(image || '', BASE_RC)
        });
  ...
  /* SearsPartsDirect */
{
  ...
  parser: async (html, q) => {
    ...
    // Модель, которую ввёл пользователь (нормализуем)
    ...
    $('.part-card, .product-card, .card, [data-component="product-card"], a[href*="/part/"], a[href*="/product/"]').each(
      (_, el) => {
        ...
        // Модель, которую мы видим в заголовке/ссылке
        const modelFromText = pnText(title + ' ' + link);
        const part_number   = modelFromText || pnFromLink(link) || pnText(title);
    ...
    if (!out.length) {
      ...
      // запасной вариант: любые ссылки /model/
      if (!out.length) {
    ...
    // ---------- ЖЁСТКАЯ ФИЛЬТРАЦИЯ ПО МОДЕЛИ ----------
    if (qModel) {
      const strict = out.filter(item => {
        const m = pnText(
          (item.title || '') +
          ' ' +
          (item.link || '') +
      ...
      // если что-то нашли с точным совпадением — оставляем только их
      if (strict.length) {
        strict.forEach(item => {
          item.part_number = qModel; // красивый вывод "Part #DLE4970W"
        });
        out = strict;
      }
    }
  ...
  /* RepairClinic */
  {
    name: 'RepairClinic',
    searchUrl: q =>
      `${BASE_RC}/Shop-For-Parts?query=${encodeURIComponent(q)}`,
    parser: async (html, q) => {
      const $ = cheerio.load(html);
      let out = [];
  ...
  /* ReliableParts */
  {
    name: 'ReliableParts',
    searchUrl: q =>
      `${BASE_RP}/catalogsearch/result/?q=${encodeURIComponent(q)}`,
    parser: async (html, q) => {
      const $ = cheerio.load(html);
      const out = [];
      ...
      $('.product-item').each((_, el) => {
        ...
        const availability = /in stock/i.test(blockText)
          ? 'In stock'
          : '';
      ...
      if (!out.length && q) {
        out.push({
          ...
          link: `${BASE_RP}/catalogsearch/result/?q=${encodeURIComponent(
            q
          )}`,
          image: '',
          source: 'ReliableParts',
  ...
  /* AppliancePartsPros */
  {
    ...
    searchUrl: q =>
      `${BASE_APP}/search.aspx?searchtext=${encodeURIComponent(q)}`,
    parser: async (html, q) => {
      const $ = cheerio.load(html);
      const out = [];
      ...
      if (!out.length && q) {
        out.push({
          ...
          link: `${BASE_APP}/search.aspx?searchtext=${encodeURIComponent(
            q
          )}`,
          image: '',
          source: 'AppliancePartsPros',
  ...
  /* PartSelect */
  {
    name: 'PartSelect',
    searchUrl: q =>
      `${BASE_PS}/Search.aspx?SearchText=${encodeURIComponent(q)}`,
    parser: async (html, q) => {
      const $ = cheerio.load(html);
      const out = [];
  ...
  /* Marcone */
  {
    name: 'Marcone',
    searchUrl: q =>
      `${BASE_MAR}/Search?query=${encodeURIComponent(q)}`,
    parser: async (html, q) => {
      const $ = cheerio.load(html);
      const out = [];
  ...
  /* eBay */
  {
    name: 'eBay',
    searchUrl: q =>
      `${BASE_EBAY}/sch/i.html?_nkw=${encodeURIComponent(q)}`,
    parser: async (html, q) => {
      const $ = cheerio.load(html);
      const out = [];
      ...
      $('li.s-item').each((_, el) => {
        ...
        const priceText = t(el$.find('.s-item__price').text());
  ...
  /* Amazon */
  {
    ...
    parser: async (html, q) => {
      ...
      $('div[data-component-type="s-search-result"]').each((_, el) => {
        ...
        const priceText =
          priceWhole || priceFrac ? `$${priceWhole}${priceFrac}` : '';
